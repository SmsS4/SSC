...
import { SEARCH_FOR_ALL, SEARCH_FOR_TAGS } from './enums';
...
function ListViewer({
  ...
  clickCallback,
    ...
    function renderAll(value) {
      ...
      if (!checkPageDone(value.writers) || !checkPageDone(value.blogs) || !checkPageDone(value.tags)){
        setFetchedAll(false)
      }else {
        setFetchedAll(true)
      }
    ...
    function renderBlogByTag(blogs) {

    }
    const renders = {
      SEARCH_FOR_ALL: renderAll,
      SEARCH_FOR_TAGS: renderTags,
      SEARCH_FOR_BLOGS: renderBlogs,
      SEARCH_FOR_WRITERS: renderWriters,
      SEARCH_FOR_BLOG_BY_TAG: renderBlogs,
    }
